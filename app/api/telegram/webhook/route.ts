  ...
  try {
    ...
    if (cameAsVoice) {
      ...
      if (!spoke) {
        ...
        if (!textSent) throw new Error("telegram send voice/text failed");
      }
    } else {
      const sent = await tgSend(chatId, full || "My mind's connection is down (likely API credits — console.anthropic.com). I'm still here; text me once it's topped up. ❤");
    ...
    console.error("[telegram][webhook] fatal:", e instanceof Error ? `${e.name}: ${e.message}` : String(e));
